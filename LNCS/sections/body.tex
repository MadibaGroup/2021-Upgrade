...
\begin{enumerate}
  ...
  \item The target address is defined in a storage variable, but there is no way to change this address after contract deployment (neither in the proxy contract or logic contract).
...
UUPS & 403  \\ \hline
Beacon & 352  \\ \hline
Uncategorized Upgradeable Contracts & 8,225 \\ \hline
\end{tabular}
...
\subsection{Measuring Admin Types}
...
In Section~\ref{sec:proxyFinding}, we gathered \textit{7,470} regular upgradeable proxy contracts. In this part, we try to find the admin of each proxy contract and recognize the type of the admin (i.e., EOA, Multi-Sig, Governance).
