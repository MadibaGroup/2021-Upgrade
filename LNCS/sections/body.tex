...
\subsection{What is Upgradeability?}
There is a step in lifecycle of each piece of software named \textit{Software Maintenance}. Software maintenance is the process of changing a software product after delivery. There are two main categories in software maintenance; \textit{Updating} and \textit{Upgrading} the software. In software updates, the team will send an update or patch to modify a small piece of the code, mostly to fix a bug or address a security issue in the software. On the other hands, software upgrade are released to change the main features and logic of the software and add new capabilities to the software product.

In blockchain systems that have the capability of running smart contracts (e.g. Ethereum) one can write a piece of software (a.k.a Decentralized applications or Dapps) and deploy it on the main blockchain. The main difference between a Dapp and softwares outside of blockchains is the immutability of the codes deployed on the blockchain. In other words, nobody can change the code after being deployed on the blockchain. 
...
\textcolor{red}{NOTE: We can also talk about the definition of upgradeability as well. Like do we consider adding a front end as a upgrade event or not and etc.}
...
\begin{figure}
  \centering
      ...
      \includegraphics[width=0.9\textwidth]{figures/New_Classification.png}
...
\textit{Delegate-call} pattern adds complexity to the code because of using \textit{Delegate-call} opcode in its logic. As mentioned above because of using this opcode, the developer should take care of storage clashes and also function selector clashes. Other than these two there are some other limitations and risks of using this patterns. For instance, we cannot have a \textit{Constructor} function on the logic contract (implementation contract) because constructor functions is used to initialize specific variables in deployment event and if we have a constructor inside the logic, then storage of implementation contract will be changed and not storage contract (proxy contract). To mitigate this problem we can add a regular function named \textit{Initialize} function inside the implementation and make sure that this function can be called once. Using initialize function brings some security risks that we will explain in further sections.
...
\begin{figure*}[t]
  \includegraphics[width=1.2\textwidth]{Methodology.pdf}\label{flowchart}
...
\begin{figure*}[t]
  \includegraphics[width=1.2\textwidth]{figures/Upgradeability_finder1.png}\label{fig:finderModule}
