...
In the further parts of this section we focus on describing the methodology used for finding upgradeable contracts use \textit{Delegatecall-based} patterns, and show the results of applying the methodology to 2 million Ethereum blocks (give time).  
...
As discussed in the classification part, the target address in beacon proxies come from an external call to another contract named \textit{Beacon Contract}. So, to find beacon upgradeable contracts we first check if the target address comes from an external call to another contract. If yes, we should just check the callee contract to find out if the target address inside the beacon contract is changeable. In case that the target address is changeable, the proxy contract is a \textit{Beacon Proxy} contract and the admin of beacon contract can change the target address inside it to upgrade the system. 

If the address does not come from an external call, then we check if there is any function inside the proxy contract that admin can call it to change the target address and upgrade the system. This is the most tricky part in our methodology to find out if there is a function inside the contract that gives the admin chance to change the target address. If that function is found we mark the proxy as an upgradeable proxy contract. The process is divided into two parts; Finding the target storage variable regarding the target address, and checking if there is any assignment to that specific storage variable inside the contract.

\paragraph{Finding storage variable (slot) of the target address.} We use bytecode decompiler named \textit{Panoramix decompiler} \footnote{\url{https://github.com/palkeo/panoramix}} to decompile the bytecode of the contract into well-formatted python language codes. Then check to find the line of the code in which the delegate call happened. The target of delegate call, which is our target address, can be to a storage variable or a storage slot and gives us the variable name or the slot of the target address which is our goal for this part.
\paragraph{Checking for assignment.} We have the decompiled code and variable name (or storage slot) of the target address. We use Regular Expression method to check if any assignment to that variable/slot happened in any function in the contract. If any assignment found, we should be sure that the other variable which is assigned to the target address variable comes from the input of that function. If these conditions are satisfied, it means that there is a function inside the contract which is able to change the target address and upgrade our system.
...
If there is no way to change the target address inside the proxy, then we pass it to another final filter. There is another way of implementing upgradeable contracts named \textit{Universal Upgradeable Proxy Standard (UUPS)} that discussed in the classification section. In this method, the target address is changeable using the implementation contract. So to filter and find them, we check the implementation contract to find out if there is any function inside the implementation contract by which the admin can change the target address inside the proxy. If yes then our proxy is a UUPS proxy contract other wise the proxy is not upgradeable. The first step here is to find the storage slot of the target address inside the proxy contract. Then we decompile the bytecode of the implementation contract and then check to find if there is any assignment to that storage slot is happened inside the implementation contract. The process of checking to find the assignment is very similar to the previous part. So if a function found that gives the admin chance to write a new amount to the storage slot regarding target address it means that the admin can call that function using the proxy to change the target address and upgrade the system. In this case we marked the proxy as UUPS proxy contract.
