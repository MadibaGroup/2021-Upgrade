...
But, not all proxy contracts are using EIP-1967 suggested storage slot. So, for non EIP-1967 proxies, we proposed a way to find the storage slot in which the admin address is stored. The process is very similar to the previous part. We first find the function in which the admin can change the \textit{target address} (upgrade function). This function is a critical function and should only be called by the admin. It means there should be an access control to check the function caller. We found this access control check and the address that is checked inside it, is the admin address.

\textbf{\emph{Finding the admin type.}} Having the admin address we can check if the account is an EOA by just checking if the account consists of a code or not\footnote{using the \textit{eth\_getCode} method for the admin address}. If the contract doesn't contain code it means that the admin is an EOA. The remained admin addresses are contract because their account keeps code. This contract can be multi-signature smart contract wallets. The most widely used multi-signature wallet is Gnosis Safe\footnote{\url{https://gnosis-safe.io/}} wallets. We automatically checked if the code of the admin address is the Gnosis wallet multi-signature and if yes we marked them as Multi-Signature admins. After picking Gnosis safe wallets we manually checked \%10 of the remained addresses to find if there is any other patterns for multi-signature wallet and we found some other patterns (e.g. MultiSignatureWalletWithDailyLimit, etc.) and add them to the dataset as well. 

A myriad of the contracts are using another proxy contract as their admin which is known as \textit{Admin Proxy}. Admin proxy contract adds another layer of indirection. In this case the real admin (owner of the admin proxy), sends their desired transaction to the admin proxy which will be redirected to the main proxy and getting executed. So, we tried to find the admin proxies among the admin addresses we have, and then find the owner of these proxies. The owner's address is the real admin account who can upgrade these systems. Now that we have the admin address we do the same processes like before to find the EOA types and the Multi-Signature types.
...
The results show that \%48 of the proxy contracts are controlled by a single EOA account and \%62 is controlled by an EOA or Multi-Signature wallets. This is a great risk to Ethereum ecosystem because in these contracts one or limited number of persons can decide to change the whole logic of the contract and take control of the funds under custody of the contract.
...
Afterwards, the attacker upgraded the proxy to the malicious implementation contract and by doing that, the huge amount of tokens were assigned to the attacker's address. Once the balance was transferred to the attacker, they upgrade the proxy back to the latest non-backdoor version to hide the exploit. 
Having huge amount of the tokens, the attacker drained liquidity from Curve Finance protocol which is a decentralized exchange.
