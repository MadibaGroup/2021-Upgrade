...
%% add this as citation to the fact that selfdestruct will be removed: 
...
% What is a rug pool and how can it happen
% 1. Backdoor in the code
% 2. Upgrade the system and add a backdoor
...
% Why admin is important and what are the types of admins
...
% The way we want to find admins
%Methodology
\subsubsection{Methodology}
...
% Boro tu folder final + ax ru takhte lab check kon marahel ro tozih bede
%Bayad proxy admin ro ham tozih bedi
% Aval EIP 1967 ke rahate bad inke chejuri mishe multi-sig ha ro peyda kard bad Proxy adminesh ke kolan chie proxy admin o ina va bad az peyda kardanesh chejuri mishe admin asli ro peyda kard + inke baghiashun governance + nn hastan
...
% Vase non EIP 1967 ha bayad begim chejuri peydashun mikonim :
% 1. Finding the upgrade function
% 2. Finding a check on caller if it is:
...
In storage variable case the first question is in which storage slot the admin address is stored. So, the first step is to find the storage slot of the admin address variable. % Caller --> admin slot || Sha3('')
For the fixed address we should find the fixed address of the admin directly.

For finding the slot of the storage variable in which admin address is saved we first find the function in which the proxy can be upgraded. For finding the upgrade function we exactly do what we did in \ref{findingProxies} part. We first find the storage variable in which we saved the implementation address and then we find a function in which the implementation address can be changed using the inputs of the function. 
...
%   a. storage variable --> Find the slot of that storage and then check the address inside it
...
%   b. If they used hash of sth (Sha3 ...)
...
%   c. If they used a fixed address as their admin
...
%Total stats
%why we cannot find some of them.
